...
svg(filename = "output/figures/03_DGE_genotype_WT_tissue/plot_DGE_number_pct.svg", width = 15, height = 10)
plot_DGE_number / plot_DGE_pct
dev.off()

shared_DGE %>% dplyr::select(gene_id, contains("4Fj")) %>% UpSetR::upset(nsets = 7)
shared_DGE %>% dplyr::select(gene_id, contains("4FsB")) %>% UpSetR::upset(nsets = 7) 
shared_DGE %>% dplyr::select(gene_id, contains("4Fk")) %>% UpSetR::upset(nsets = 7) 
shared_DGE %>% dplyr::select(gene_id, contains("4FsA")) %>% UpSetR::upset(nsets = 7) 
...
gprofiler2_upregulated_plots$`4Fj.vs.WT_Kidney`
gprofiler2_downregulated_plots$`4Fj.vs.WT_Kidney`


pdf(file = "output/figures/03_DGE_analysis/median_expression_plot.pdf", width = 8, height = 7)
median_expression_plot
dev.off()

pdf(file = "output/figures/03_DGE_analysis/enrichGO_clusters.pdf", width = 13, height = 7)
enrichGO_clusters_dotplot$enrichGO_clusters_upregulated + enrichGO_clusters_dotplot$enrichGO_clusters_downregulated
dev.off()

pdf(file = "output/figures/03_DGE_analysis/volcano_plots.pdf", width = 21, height = 10)
patchwork::wrap_plots(volcano_plots)
dev.off()

pdf(file = "output/figures/03_DGE_analysis/gprofiler2_plot1.pdf", width = 15, height = 8)
(gprofiler2_upregulated_plots$`4F D1 +HS 4h vs. Control D1 +HS 4h` / (ggplot() + theme_void())) + plot_layout(heights = c(0.1, 1)) |
(gprofiler2_downregulated_plots$`4F D1 +HS 4h vs. Control D1 +HS 4h`)
dev.off()

pdf(file = "output/figures/03_DGE_analysis/gprofiler2_plot2.pdf", width = 9, height = 5)
gprofiler2_downregulated_plots$`4F D1 +HS 48h vs. Control D1 +HS 48h`
dev.off()

pdf(file = "output/figures/03_DGE_analysis/gprofiler2_plot3.pdf", width = 7, height = 2.5)
gprofiler2_upregulated_plots$`4F D10 -HS 48h vs. Control D10 -HS 48h`
dev.off()
