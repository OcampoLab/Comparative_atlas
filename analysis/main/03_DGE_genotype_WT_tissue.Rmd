...
furrr::future_map(c("4Fj", '4FsB', "4Fk", "4FsA") %>% set_names(., paste(., "upregulated")), function(x){
  ...
  names(DGE_results_list) <- 
    ...
    gprofiler2::gost(query                       = DGE_genotype_WT_tissue_results$`4Fj.vs.WT_Heart`$geneID_upregulated, 
                   organism                    = "mmusculus", 
                   significant                 = T, 
                   user_threshold              = 0.05, 
...
gprofiler2_upregulated_cluster1 <- 
  gprofiler2::gost(query                       = DGE_genotype_WT_tissue_results$`4Fj.vs.WT_Heart`$geneID_upregulated, 
                   organism                    = "mmusculus", 
                   significant                 = T, 
                   user_threshold              = 0.05, 
...
gprofiler2_upregulated_cluster1_plot <- 
gprofiler2_upregulated_cluster1$result %>% 
  mutate(p_adjust = p.adjust(p_value, "bonferroni")) %>% 
  filter(p_adjust < 0.05) %>% 
  filter(term_size < 500) %>% 
  filter(!source %in% c("TF")) %>% 
  ggplot(aes(-log10(p_adjust), term_name, fill = -log10(p_adjust))) + geom_col() + facet_grid(source ~ ., scales = "free", space = "free") + theme_publication() + scale_fill_distiller(palette = "Reds", direction = 1) + theme(strip.text.y = element_text(angle = 0, hjust = 0))
