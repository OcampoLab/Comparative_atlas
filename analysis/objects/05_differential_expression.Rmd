...
subtitle: "Find differential expressed genes"
...
stopifnot(identical(metadata_subset$`Sample ID`, colnames(tximport_object$counts)))
...
map(list("4Fj.vs.WT"  = c("4Fj",  "WT"),
  ...
  deseq2_object <- deseq2_object[,colnames(deseq2_object) %in% metadata_subset[metadata_subset$`Sample ID` %in% sample_subset, "Sample ID", drop = T]]
  deseq2_object$Genotype <- factor(deseq2_object$Genotype, levels = x)
...
map(list("4Fj.vs.WT_Kidney"          = c("4Fj",  "WT", "Kidney"),
  ...
  # Subset the DESeq2 object and create new Genotype levels
  deseq2_object <- deseq2_object[,colnames(deseq2_object) %in% metadata_subset[metadata_subset$`Sample ID` %in% sample_subset, "Sample ID", drop = T]]
  deseq2_object$Genotype <- factor(deseq2_object$Genotype, levels = x[1:2])
...
map(list(
  ...
  # Subset the DESeq2 object and create new Genotype levels
  deseq2_object <- deseq2_object[,colnames(deseq2_object) %in% metadata_subset[metadata_subset$`Sample ID` %in% sample_subset, "Sample ID", drop = T]]
  deseq2_object$Tissue <- factor(deseq2_object$Tissue, levels = c(x[2], "Other tissues"))
